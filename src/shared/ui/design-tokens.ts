import type { DesignTokens } from "@shared/types/portfolio";

export const designTokens: DesignTokens = {
  color: {
    background: "#f5f5f0",
    surface: "#fffdf8",
    surfaceMuted: "#f0eadf",
    surfaceStrong: "#e3dac9",
    foreground: "#111111",
    muted: "#5f5a52",
    accent: "#8a6a2f",
    accentSoft: "#d8c39a",
    border: "#d8d2c8",
    overlay: "rgba(17, 17, 17, 0.72)",
  },
  spacing: {
    "2xs": "0.125rem",
    xs: "0.25rem",
    sm: "0.5rem",
    md: "1rem",
    lg: "1.5rem",
    xl: "3rem",
    "2xl": "4.5rem",
  },
  radius: {
    sm: "0.5rem",
    md: "1rem",
    lg: "1.5rem",
    pill: "999px",
  },
  layout: {
    narrow: "42rem",
    content: "72rem",
    wide: "96rem",
  },
  typography: {
    display: "'Space Grotesk', 'Segoe UI', sans-serif",
    body: "'IBM Plex Sans', 'Segoe UI', sans-serif",
    mono: "'IBM Plex Mono', 'Consolas', monospace",
    lineHeight: {
      snug: "1.1",
      body: "1.6",
    },
    scale: {
      xs: "0.75rem",
      sm: "0.875rem",
      md: "1rem",
      lg: "1.5rem",
      xl: "2.5rem",
      "2xl": "3.5rem",
    },
  },
};

export const designTokenCssVariables = {
  "--color-background": designTokens.color.background,
  "--color-surface": designTokens.color.surface,
  "--color-surface-muted": designTokens.color.surfaceMuted,
  "--color-surface-strong": designTokens.color.surfaceStrong,
  "--color-foreground": designTokens.color.foreground,
  "--color-muted": designTokens.color.muted,
  "--color-accent": designTokens.color.accent,
  "--color-accent-soft": designTokens.color.accentSoft,
  "--color-border": designTokens.color.border,
  "--color-overlay": designTokens.color.overlay,
  "--space-2xs": designTokens.spacing["2xs"],
  "--space-xs": designTokens.spacing.xs,
  "--space-sm": designTokens.spacing.sm,
  "--space-md": designTokens.spacing.md,
  "--space-lg": designTokens.spacing.lg,
  "--space-xl": designTokens.spacing.xl,
  "--space-2xl": designTokens.spacing["2xl"],
  "--radius-sm": designTokens.radius.sm,
  "--radius-md": designTokens.radius.md,
  "--radius-lg": designTokens.radius.lg,
  "--radius-pill": designTokens.radius.pill,
  "--layout-narrow": designTokens.layout.narrow,
  "--layout-content": designTokens.layout.content,
  "--layout-wide": designTokens.layout.wide,
  "--font-display": designTokens.typography.display,
  "--font-body": designTokens.typography.body,
  "--font-mono": designTokens.typography.mono,
  "--line-height-snug": designTokens.typography.lineHeight.snug,
  "--line-height-body": designTokens.typography.lineHeight.body,
  "--font-size-xs": designTokens.typography.scale.xs,
  "--font-size-sm": designTokens.typography.scale.sm,
  "--font-size-md": designTokens.typography.scale.md,
  "--font-size-lg": designTokens.typography.scale.lg,
  "--font-size-xl": designTokens.typography.scale.xl,
  "--font-size-2xl": designTokens.typography.scale["2xl"],
} as const;
